  ...
  function App() {
    ...
    const handleSubmitExchangeRate = async (e) => {
      ...
      try {
        ...
        const response = await fetch("/api/addExchangeRate", {
          method: "POST",
          headers: {
    ...
    const handleSubmitGetAmount = async (e) => {
      ...
      try {
        // console.log(react_app_url);
        const response = await fetch(`/api/getAmount?sourceCurrency=${formData.sourceCurrency}&targetCurrency=${formData.targetCurrency}`);
